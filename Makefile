...
UNAME := $(shell uname)
ifeq ($(UNAME), Darwin)
	_CP_COMMAND := cp target/debug/libsuperstore.dylib superstore/superstore.abi3.so
else
	_CP_COMMAND := cp target/debug/libsuperstore.so superstore/superstore.abi3.so
endif
